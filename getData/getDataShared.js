...
module.exports = class GetDataShared extends Mysql {
     ...
     * @desc 获取伤害的倍数
     */
    InjuryMultiple(Type, firstType, lastType = '') {
        const last = lastType ? this.type(lastType) : -1;
        const one = attack[this.type(Type)][this.type(firstType)];
        const tow = last !== -1 ? attack[this.type(Type)][last] : 0;
        return one + tow;
    }
